...
function startSauce {
  "${SC_BINARY}" \
    ...
    -vv \
    -l "$(pwd)/reports/sauce/sauce_connect.$(echo "${PACKAGE}" | awk -F '/' '{ print $NF }').${SUITE_ITERATION}.${SC_ITERATION}.log" \
...
if [ "${EXIT_CODE}" -ne "0" ]; then
  if [ -f "./reports/junit/*/${PACKAGE}-karma.xml" ]; then
    log "Suite consistently exited non-zero, but produced an xml file so not failing the build"
    exit 0
  fi
fi
