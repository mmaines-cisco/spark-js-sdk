  ...
  createOffer() {
    if (!this.peer) {
      ...
      this.peer.ontrack = (event) => {
        ...
        // The track event will fire twice for an audio/video call, but since
        // it'll contain the same stream in both cases, we won't know about it.
        const willNotFire = this.remoteMediaStream === event.streams[0];
        this.remoteMediaStream = event.streams[0];
        if (willNotFire) {
          console.log(`synthetic change event`);
          this.trigger(`change:remoteMediaStream`);
          console.log(`synthetic change event success`);
        }
