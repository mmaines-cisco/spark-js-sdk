...
browserOnly(describe)(`plugin-phone`, function() {
  ...
  describe(`Phone`, () => {
    let mccoy, spock, checkov;
    before(`create users and register`, () => testUsers.create({count: 3})
      .then((users) => {
        [mccoy, spock, checkov] = users;
    ...
    describe(`when a call is received`, () => {
      ...
      it(`does not emit multiple call:incoming events for the same call`, () => {
        mccoy.spark.internal.mercury.on(`all`, (eventName, event) => {
          if (eventName.startsWith(`event`)) {
            try {
              const _ = require(`lodash`);
              console.info(`XXXX`, eventName, _.get(event, `data.locus.self.alertType.action`), _.get(event, `data.locus.self.state`));
            }
            catch (err) {
        ...
        return handleErrorEvent(spock.spark.phone.dial(mccoy.email), (call) => {
          let mccoyCall;
          call.on(`all`, (eventName) => console.info(`EVENT`, eventName));
          return call.when(`change:locus`).then(() => {
          ...
          .then(() => {
            ...
            .then(() => {
              console.info(`000`);
              return Promise.all([
                mccoyCall.when(`remoteAudioMuted:change`),
                // maxWaitForEvent(1000, `remoteAudioMuted:change`, mccoyCall),
