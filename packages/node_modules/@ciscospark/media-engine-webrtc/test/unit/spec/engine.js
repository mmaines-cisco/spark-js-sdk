            ...
            videoEndStates.forEach((videoEnd) => {
              ...
              describe(message, () => {
                ...
                it(`initiates a session with audio=${audioStart} and video=${videoStart}`, () => {
                  logRemoteState(`test start`, engine);
                  logSendingState(`test start`, engine);
                  ...
                  logRemoteState(`after set media`, engine);
                  logSendingState(`after set media`, engine);

                  return engine.createOffer()
                    .then((offer) => {
                      logRemoteState(`after create offer`, engine);
                      logSendingState(`after create offer`, engine);
                    ...
                    .then(mockAnswer)
                    .then((answer) => {
                      logRemoteState(`before accept answer`, engine);
                      logSendingState(`before accept answer`, engine);
                      assertLocalMedia(engine, audioStart, videoStart);
                    ...
                    .then((answer) => Promise.all([
                      (audioStart.includes(`recv`) || videoStart.includes(`recv`)) && expectEvent(5000, `track`, engine),
                      engine.acceptAnswer(answer)
                    ]))
                    .then(() => {
                      logRemoteState(`after accept answer`, engine);
                      logSendingState(`after accept answer`, engine);
                    })
                    .then(() => {
                      assertLocalMedia(engine, audioStart, videoStart);
                ...
                it(`transitions to audio=${audioEnd} and video=${videoEnd}, ${expectToRenegotiate ? `expecting` : `not expecting`} to renegotiate ${expectNewTrack ? `and expecting` : `but not expecting`} a new remote track`, () => {
                  // TODO need to assert we don't rennegotiate
                  logRemoteState(`test start`, engine);
                  logSendingState(`test start`, engine);

                  engine.setMedia(`audio`, audioEnd);
                  logRemoteState(`after set audio`, engine);
                  logSendingState(`after set audio`, engine);

                  engine.setMedia(`video`, videoEnd);

                  logRemoteState(`after set video`, engine);
                  logSendingState(`after set video`, engine);
                  return Promise.all([
                    expectToRenegotiate && expectEvent(5000, `negotiationneeded`, engine)
                      .then(() => retry(() => assertLocalMedia(engine, audioEnd, videoEnd)))
                      .then(() => {
                        logRemoteState(`before create offer`, engine);
                        logSendingState(`before create offer`, engine);
                      })
                    ...
                    expectNewTrack && expectEvent(5000, `track`, engine)
                  ])
                  .then(() => {
                    logRemoteState(`after accept answer`, engine);
                    logSendingState(`after accept answer`, engine);
                ...
                it(`returns to audio=${audioStart} and video=${videoStart}`, () => {
                  ...
                  return Promise.resolve(expectToRenegotiateOnReturn && expectEvent(5000, `negotiationneeded`, engine)
                    .then(() => engine.createOffer())
                    .then((offer) => {
                      logRemoteState(`after create offer`, engine);
                      logSendingState(`after create offer`, engine);
                    ...
                    .then((answer) => Promise.all([
                      (needsRemoteAudioTrack || needsRemoteVideoTrack) && expectEvent(5000, `track`, engine),
                      engine.acceptAnswer(answer)
                    ])))
                    .then(() => {
                      logRemoteState(`after accept answer`, engine);
      ...
      it(`adds a new track to an inprogress connection`, () => {
        ...
        return engine.createOffer()
          ...
          .then((stream) => {
            ...
            engine.setMedia(`audio`, `sendrecv`, track);
            return expectEvent(5000, `negotiationneeded`, engine);
          })
          .then(() => engine.createOffer())
          .then(mockRenegotiate)
          .then((answer) => engine.acceptAnswer(answer))
...
function logSendingState(name, engine) {
  console.log(name, require(`util`).inspect({
    lmsTracks: engine.localMediaStream.getTracks().length,
    pcSenders: engine.pc.getSenders().length,
    pcStreams: engine.pc.getLocalStreams().length
  }));
}

function logRemoteState(name, engine) {
  console.log(name, require(`util`).inspect({
    rmsTracks: engine.remoteMediaStream.getTracks().length,
    pcReceivers: engine.pc.getReceivers().length,
    pcStreams: engine.pc.getRemoteStreams().length
  }, {depth: null}));
}
