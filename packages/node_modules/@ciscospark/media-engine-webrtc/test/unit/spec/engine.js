...
browserOnly(describe)(`media-engine-webrtc`, () => {
            ...
            videoEndStates.forEach((videoEnd) => {
              ...
              describe(message, () => {
                ...
                it(`transitions to audio=${audioEnd} and video=${videoEnd}`, () => {
                  ...
                  return Promise.all([
                    ...
                    expectNewTrack && Promise.race([
                      ...
                      new Promise((resolve, reject) => setTimeout(() => reject(new Error(`Expected track not added`)), 20000))
                    ])
                  ])
                  .then(() => {
                    assertLocalMedia(engine, audioEnd, videoEnd);
                ...
                it(`returns to audio=${audioStart} and video=${videoStart}`, () => {
                  ...
                  console.info(`@@@@@@@@@`, expectToRenegotiateOnReturn);
                  console.info(needsLocalAudioTrack, needsLocalVideoTrack, needsRemoteAudioTrack, needsRemoteVideoTrack, expectToRenegotiateOnReturn, engine.remoteMediaStream.getAudioTracks().length, engine.remoteMediaStream.getVideoTracks().length);
                  engine.setMedia(`audio`, audioStart);
                  engine.setMedia(`video`, videoStart);
...
function assertRemoteKind(kind, direction, engine) {
  ...
  if (direction.includes(`recv`)) {
    console.info(engine.pc.getRemoteStreams().length);
    console.info(engine.pc.getRemoteStreams()[0].getVideoTracks().length);
