    ...
    describe(`#connect()`, () => {
      ...
      describe(`when \`maxRetries\` is set`, () => {
        ...
        skipInBrowser(it)(`fails after \`maxRetries\` attempts`, () => {
          spark.config.mercury.maxRetries = 2;
          socketOpenStub.restore();
          ...
          return promiseTick(4)
            ...
            .then(() => {
              clock.tick(spark.config.mercury.backoffTimeReset);
              return promiseTick(4);
            })
            .then(() => {
              assert.calledTwice(Socket.prototype.open);
              clock.tick(2 * spark.config.mercury.backoffTimeReset);
              return promiseTick(4);
            })
            .then(() => {
              assert.calledThrice(Socket.prototype.open);
              clock.tick(4 * spark.config.mercury.backoffTimeReset);
      ...
      skipInBrowser(describe)(`when the connection fails`, () => {
        it(`backs off exponentially`, () => {
          ...
          return promiseTick(4)
            ...
            .then(() => {
              assert.calledThrice(Socket.prototype.open);
              clock.tick(8 * spark.config.mercury.backoffTimeReset);
              return promiseTick(4);
            })
            .then(() => {
              assert.calledThrice(Socket.prototype.open);
