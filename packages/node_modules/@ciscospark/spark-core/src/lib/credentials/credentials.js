  ...
  initialize(attrs, options) {
    ...
    this.listenToOnce(this.parent, `change:config`, () => {
      if (this.config(`authorizationString`)) {
        ...
        /* eslint-disable camelcase */
        this.spark.config.credentials.client_id = parsed.query.client_id;
        this.spark.config.credentials.redirect_uri = parsed.query.redirect_uri;
        this.spark.config.credentials.scope = parsed.query.scope;
        this.spark.config.credentials.authorizeUrl = parsed.href.substr(0, parsed.href.indexOf(`?`));
        /* eslint-enable camelcase */
      }
    });
