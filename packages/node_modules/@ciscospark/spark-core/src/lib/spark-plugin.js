...
const SparkPlugin = AmpState.extend({
  ...
  initialize(...args) {
    ...
    this.listenToAndRun(this.spark, `change:config`, () => {
      const namespace = this.getNamespace();
      if (namespace) {
        let cfg = get(this.spark, `config.${namespace.toLowerCase()}`);
        if (!cfg) {
          cfg = {};
          set(this.spark, `config.${namespace.toLowerCase()}`, cfg);
        }
        this.config = cfg;
      }
    });
    ...
    this.on(`change`, (model, options) => {
      if (this.parent) {
        this.parent.trigger(`change:${(this.getNamespace() || this.parent.getNamespace()).toLowerCase()}`, this.parent, this, options);
      }
    });
  },
