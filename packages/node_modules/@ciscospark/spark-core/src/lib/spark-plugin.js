...
const SparkPlugin = AmpState.extend({
  ...
  config(keyPath) {
    ...
    if (!namespace) {
      throw new Error(`Cannot call SparkPlugin#config() without specifying your plugin's namespce`);
    }
    const fullKeyPath = `${namespace.toLowerCase()}.${keyPath}`;
    console.info(fullKeyPath);
    return get(this.spark.config, fullKeyPath);
  },
