...
describe(`spark-core`, () => {
  describe(`Credentials`, () => {
    describe(`Token`, () => {
      ...
      beforeEach(() => {
        spark = new MockSpark();
        spark.config.credentials.client_id = `fake`;
        spark.config.credentials.client_secret = `fake`;
      });
      ...
      describe(`#canDownscope`, () => {
        it(`indicates if this token can be used to get a token of lesser scope`, () => {
          ...
          const cfg = cloneDeep(spark.config);
          cfg.credentials.client_id = `blarg`;
          spark.config = cfg;
      ...
      describe(`#initialize()`, () => {
        ...
        it(`requires an access token`, () => {
          assert.throws(() => {
            ...
            const x = new Token(undefined, {parent: spark});
          }, /`access_token` is required/);

          assert.doesNotThrow(() => {
            // eslint-disable-next-line no-unused-vars
            const x = new Token({access_token: `AT`}, {parent: spark});
          }, /`access_token` is required/);
        });
